#!/usr/bin/env python
#
# Modified by Vijay Govindarajan from carla_manual_control.py
#
...
"""
Welcome to CARLA ROS network control.
...
# locally adjust path for imports or add fast-semantic-segmentation to PYTHONPATH
filepath = os.path.dirname(os.path.realpath(__file__))
sys.path.insert(0, filepath+"/fast-semantic-segmentation")
from protos import pipeline_pb2
from builders import model_builder
from libs.exporter import deploy_segmentation_inference_graph
from libs.constants import CARLA_LABEL_COLORS, CARLA_LABEL_IDS
...
class NetworkControl(object):
    ...
    def __init__(self, role_name, hud, model_config, checkpoint, use_label_colors=True):
        self.role_name = role_name
        self.hud = hud
        ...
        self._control.manual_gear_shift = 0
        ...
        # set up the network here, image callback needed
        pipeline_config = pipeline_pb2.PipelineConfig()
        with tf_ml.gfile.GFile(model_config, 'r') as f:
            text_format.Merge(f.read(), pipeline_config)

        input_shape  = [600, 800, 3]
        pad_to_shape = [601, 801, 3]
        self.label_map = (CARLA_LABEL_COLORS
            if use_label_colors else CARLA_LABEL_IDS)
        ...
        self.outputs, self.placeholder_tensor = deploy_segmentation_inference_graph(
            ...
            label_color_map=self.label_map)

        self.session = tf_ml.Session()
        input_graph_def = tf_ml.get_default_graph().as_graph_def()
        saver = tf_ml.train.Saver()
        ...
        self.carla_rgb_img_subscriber = rospy.Subscriber(
            "/carla/{}/camera/rgb/front/image_color".format(self.role_name), Image, self._on_image_received)

        self.carla_seg_img_publisher = rospy.Publisher(
            "/carla/{}/camera/rgb/front/image_segpred".format(self.role_name), Image, queue_size=1)

        #self.loop()
        # control can be done in parse events
    ...
    #         # run the network and publish the result
    #         predictions = self.session.run(self.outputs,
    #             feed_dict={self.placeholder_tensor: img})
           
    #         predictions = predictions.astype(np.uint8)
    #         if len(self.label_map[0]) == 1:
    #            predictions = np.squeeze(predictions,-1)
    #            img_format = "mono8"
    #         else:
    #             img_format = "rgb8"

    #         imgmsg = self.bridge.cv2_to_imgmsg(predictions[0], img_format)
    #         imgmsg.header = header
    #         self.carla_seg_img_publisher.publish(imgmsg)
    #         r.sleep()
    ...
    def parse_events(self, clock):
        ...
        self._control.reverse = 0 # only forward driving

    def _on_image_received(self, image):
        """
        Callback when receiving a camera RGB image to process.
        """
        array = numpy.frombuffer(image.data, dtype=numpy.dtype("uint8"))
        array = numpy.reshape(array, (image.height, image.width, 4))
        array = array[:, :, :3]
        array = array[:, :, ::-1] # bgr -> rgb
        #array = cv2.resize(array, (image.height / 2, image.width /2), interpolation=cv2.INTER_AREA)

        with self.image_lock:
            self.latest_image = array
            self.latest_img_header = image.header
...
def main():
    ...
    main function
    """
    rospy.init_node('carla_network_control', anonymous=True)

    role_name = rospy.get_param("~role_name", "ego_vehicle")
    model_config = rospy.get_param("~model_config")
    ...
    pygame.init()
    pygame.font.init()
    pygame.display.set_caption("CARLA ROS network control")
    world = None
    try:
        ...
        controller = NetworkControl(role_name, hud, model_config, checkpoint)
