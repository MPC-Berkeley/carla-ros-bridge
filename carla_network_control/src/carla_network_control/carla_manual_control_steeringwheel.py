...
class DualControl(object):
    """
    Handle input events from steering wheel/joystick.
    """
    ...
    def __init__(self, role_name, hud):
        ...
        # Axis definitions (G27 racing wheel)
        self._steer_idx = int(0)
        self._throttle_idx = int(2)
        self._brake_idx = int(3)

        # Button definitions (G27 racing wheel, see parse code for details).
        self._handbrake_idx = int(4)
        self._left_intent_idx = int(1)
        self._right_intent_idx = int(2)
    ...
    def _parse_vehicle_wheel(self):
        ...
        #print(jsButtons)
        '''
        Button mapping.  * prefix indicates it is being used by this code.
        0: steering wheel red buttons middle and bottom (both sides); black circle button bottom on shifter
        1: black circle button left on shifter
        2: black circle button right on shifter
        3: black circle button top on shifter
        *4: right steering wheel paddle (default for handbrake)
        5: left steering wheel paddle
        6: steering wheel red button top right
        7: steering wheel red button top left
        8: red middle left circle button on shifter
        9: red middle right circle button on shifter
        10: red right circle button on shifter
        11: red left circle button on shifter
        12: ?
        13: ?
        14: gear in drive position
        15: gear in reverse position
        14 and 15 are both zero if in a neutral position.
        '''

        # Custom function to map range of inputs [1, -1] to outputs [0, 1] i.e 1 from inputs means nothing is pressed        
        # Tuned by VG and IB for mapping G27 wheel range to physical steer limits of the car.
        # Also added a bias in throttle to compensate for drag (like in a real car).
        K1 = 0.55
        ...
        if bool(jsButtons[self._left_intent_idx]):
            self._control.intention = -1
        elif bool(jsButtons[self._right_intent_idx]):
            self._control.intention = 1
        else:
            self._control.intention = 0
